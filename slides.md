...
<!--
昨年APIシナリオテストを書くべき１０の理由という内容で登壇してきました。
APIシナリオテストを知らない方向けになっていますので是非ご覧ください
-->
...
<!--
カバレッジとは網羅率を指していて、どれだけテストをしたかの指標となります
-->
...
<!--
ここで重要なのが、APIテストのテスト観点になりますが、大きく２つの観点にわかれます。
１つ目はエンドポイントの仕様の正確性を担保するもの
２つ目はAPIをチェーンで呼び出してのユースケース観点での動作を担保するもの
です
-->
...
<!--
API仕様書に記載されているエンドポイントに対してのテスト網羅性を確認します
-->
...
<!--
こちらの図にあるようにエンドポイントに対してカバーしているテストケース数を把握します
こちらはAPIシナリオテストツールのrunnを使うことで見ることができます
-->
...
<!--
エンドポイントのカバー率だけなの？リクエストパラメータの組み合わせは見なくていいか？と疑問に持たれる方は多いかと思います。
-->
...
<!--
こちらは組み合わせのテストパターンは見る必要があります
ただガッツリやるならControllerテストでやるのをおすすめします。
APIテストでは最低限Exampleや必須項目のみのテストをやるだけでも品質が担保できるのでオススメです。
-->
...
<!--
先ほど出てきたrunn(ランエヌ)というツールに対してどうなの？と思われる方が多いと思います
-->
...
<!--
書き味が大変良いです！
テキストベースでシナリオを書くことができてコピペで量産し易いのが気に入っています。
-->
...
<!--
runnの魅力を詰め込んだチュートリアル本を作成しましたので、是非見てください！
-->
...
<!--
２つ目になりますが、ユースケースでの網羅性を見ていきますが、
ユースケース毎にエンドポイントがかわるので、先程紹介したエンドポイントのカバー率で事足りると個人的に感じています。
-->
...
<!--
先に結果から見ていきます
このような形でカバレッジを取ることができました
runnを使ってテストした結果となります。
...
runnにはリクエストにカスタムヘッダーをつけるという機能が存在しており、
ヘッダー情報としてシナリオのidとステップが確認できるようになっています。
またidを指定して再実行するという機能があります
-->
...
<!--
技術的な詳細を見ていきたいと思いますが、Xdebugを使います！
こちらに記載した通りにゴニョゴニョすればみれるようになります。
-->
...
<!--
ただ毎回Xdebugを仕込むのが面倒だと思ったので、 middlewareを作成しました。
Laravel向けにはなりますが、composer化して導入は楽なので是非試してみてください
-->
